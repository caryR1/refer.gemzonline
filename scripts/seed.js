...
/**
 * Starter data: the tenant, relationship options, a sample campaign with two
 * commission ranks and three reminder slots, and the default email templates.
 *
 *   npm run db:seed
 *
...
const relations = require('../src/lib/relations');
...
/**
 * Default templates. Email bodies are plain HTML — they are dropped into the
 * styled shell at send time, so no wrapper markup is needed here.
 */
const TEMPLATES = [
  {
...
async function main() {
  ...
  await relations.seedDefaults(t.id);
  const relCount = await db.one('select count(*)::int as n from relation_options where tenant_id = $1', [t.id]);
  console.log(`Relationship options: ${relCount.n}`);

  // --- sample campaign -----------------------------------------------------
  let campaign = await db.one('select * from campaigns where tenant_id = $1 and slug = $2', [t.id, CAMPAIGN.slug]);

  if (!campaign) {
    campaign = await db.one(
      ...
      [
        t.id, CAMPAIGN.name, CAMPAIGN.slug, CAMPAIGN.client_name, CAMPAIGN.description,
    ...
    console.log(`Campaign created: ${campaign.name}  →  /r/${campaign.slug}`);
  } else {
    console.log(`Campaign already present: ${campaign.name}`);
  }
  ...
  for (const rank of RANKS) {
    await db.query(
      ...
      [
        t.id, campaign.id, rank.name, rank.description, rank.is_default, rank.rank_order,
  ...
  const ranks = await db.all('select name, is_default from commission_profiles where campaign_id = $1 order by rank_order', [campaign.id]);
  console.log(`Commission ranks: ${ranks.map((r) => r.name + (r.is_default ? ' (default)' : '')).join(', ')}`);

  // --- reminder slots (all off) -------------------------------------------
  const defaults = [
  ...
  for (const r of defaults) {
    await db.query(
      ...
      [t.id, campaign.id, r.slot, r.offset_value, r.offset_unit]
    );
  }
  console.log('Reminder slots: 3 created, all switched off');
  ...
  for (const tpl of TEMPLATES) {
    await db.query(
      ...
      [t.id, tpl.channel, tpl.name, tpl.slug, tpl.trigger_event, tpl.send_to, tpl.subject, tpl.body_html]
    );
  }
  const tplCount = await db.one('select count(*)::int as n from notification_templates where tenant_id = $1', [t.id]);
  console.log(`Message templates: ${tplCount.n}`);

  // --- settings ------------------------------------------------------------
  await db.query(
    `insert into settings (tenant_id, key, value) values
       ($1,'pipeline','{"stale_lead_days":14}'::jsonb)
     on conflict (tenant_id, key) do nothing`,
    [t.id]
  );

  console.log(`\nSeed complete. Public link: ${config.appUrl}/r/${campaign.slug}`);
  console.log('Next: npm run create:admin -- you@example.com "Your Name" "a-strong-password"');
}
